...
def main_mol_tree(oinput, ovocab, MAX_TREE_WIDTH=50, csv=False):
    cset = set()
    if csv:
        smiles = pd.read_csv(oinput)["smiles"].tolist()

    with open(oinput, 'r') as input_file:
        if csv:
            data = pd.read_csv(input_file)["smiles"].tolist()
        else:
...
if __name__ == "__main__":
    ...
    main_mol_tree(opts.input, opts.vocab, opts.csv)
